...
export function SettingsPage({ settings, reload }: { settings: Settings; reload: () => Promise<void> }) {
  ...
  return <>
    ...
    <section className="settings-section email-settings">
      ...
      <div className="setting-row"><span className="setting-icon email"><Mail /></span><span><strong>Sending account</strong><small>Email asks for approval by default. A Send Email workflow node can explicitly opt into automatic sending.</small></span></div>
      <div className="integration-form">
        ...
        <Notice><span>{emailForm.provider === 'gmail' ? <>Gmail requires a 16-character <a href="https://myaccount.google.com/apppasswords" target="_blank" rel="noreferrer">Google App Password</a> for SMTP—not your normal Google password. Turn on 2-Step Verification first, create an App Password, paste it above, save, then send a test.</> : emailForm.provider === 'outlook' ? 'Microsoft accounts may require SMTP AUTH or an app password enabled by the account administrator.' : emailForm.provider === 'yahoo' ? 'Yahoo normally requires an app password rather than the regular account password.' : 'Use the SMTP server, port, security mode, and credentials supplied by the email provider.'}</span></Notice>
    ...
    <section className="license-box"><strong>Local Agent Studio 0.5.2</strong><p>Apache-2.0 licensed and provided “as is.” No software can guarantee freedom from vulnerabilities or data loss. Keep independent backups, review approvals and generated output, and use external providers, code, MCP servers, and email at your own risk to the maximum extent permitted by law. Model weights and external services retain their own terms. See DISCLAIMER.md in the project repository.</p></section>
  </>;
}
